...
@section('content')
    ...
    <div class="max-w-[85rem] py-5 lg:py-7 mx-auto">
        <div class="sm:w-11/12 lg:w-3/4 mx-auto">
            ...
            <form method="POST" class="card" action="{{ route($routePath . '.update', ['ose' => $item]) }}">
                ...
                @foreach ($servers as $server)
                    ...
                    @include('admin/shared/select', [
                        ...
                        'label' => __('proxmox::messages.oses.oses'),
                        'name' => 'osnames['. $server->id .'][]',
                        'options' => $oses[$server->id] ?? [],
                        'value' => old('osename['. $server->id .']', $osenames[$server->id] ?? [])
                    ])
                @endforeach
