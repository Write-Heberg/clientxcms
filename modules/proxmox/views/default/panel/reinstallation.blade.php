            ...
            <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="card">
                    ...
                    <span class="flex items-center text-1xl uppercase font-medium text-gray-900 dark:text-white mb-4">
                        ...
                        <p class="ml-2">
                            {{ __('proxmox::messages.reinstallation.title') }} - {{ $vps->hostname() }}
                        </p>
                    </span>
                        @if ($vps->canReinstall($service) && $vps->hasReinstallLimit($service))
                        <h3 class="text-xs font-semibold text-gray-600 dark:text-gray-400">
                            {{ __('proxmox::messages.reinstallation.limit_text', ['current' => $vps->getConfigMetadata($service)->current_reinstall ?? 0, 'max' => $vps->getConfigMetadata($service)->max_reinstall ?? 0]) }}
                        </h3>
                        @endif
                    </div>
                    <form method="POST" action="{{ route('proxmox.reinstall', ['service' => $service]) }}">
                    <div class="grid">
            ...
            <div class="text-2xl text-gray-600 dark:text-gray-400" style="font-size: 15px;">
                ...
                {{ $template->name }}
                        ...
                        <button type="submit" class="btn btn-primary mt-4">
                            {{ __('proxmox::messages.reinstallation.submit') }}
                        </button>
                </div>
                </form>
            </div>
