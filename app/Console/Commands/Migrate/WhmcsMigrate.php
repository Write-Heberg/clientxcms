...
class WhmcsMigrate extends Command
{
    ...
    public function handle()
    {
        ...
        if ($this->option('force')) {
            ...
            if ($this->option('all') || $this->option('servers')) {
                Server::truncate();
            }
            if ($this->option('all') || $this->option('clients')) {
                Customer::truncate();
            }
            if ($this->option('all') || $this->option('groups')) {
                Group::truncate();
            }
            ...
            if ($this->option('all') || $this->option('services')) {
                Service::truncate();
            }
        ...
        if ($this->option('all') || $this->option('pterodactyl')) {
            $this->migratePterodactylConfig();
        }
        if ($this->option('all') || $this->option('wisp')) {
        ...
        foreach ($results as $result) {
            try {
                ...
                PterodactylConfig::insert([
                    ...
                    'eggs' => json_encode([join(PterodactylConfig::DELIMITER, [$result->configoption7, $result->configoption8])]),
                    ...
                    'databases' => $result->configoption14,
        ...
        foreach ($results as $result) {
            try {
                ...
                $service = Service::create([
                    ...
                    'expires_at' => $expires,
                    'notes' => $result->notes,
                    'created_at' => $result->regdate,
                    'updated_at' => now(),
                ]);
                $service->id = $result->id;
                $service->created_at = $result->regdate;
                $service->save();
        ...
        foreach ($results as $result) {
            try {
                ...
                $invoice = Invoice::create([
                    ...
                    'external_id' => $transid,
                    'fees' => $fees,
                    'notes' => $result->notes . ' Imported from WHMCS',
                    'paymethod' => $gateway,
