                    ...
                    <span class="block px-6 py-2">
                      <span class="text-sm text-gray-600 dark:text-gray-400">
                          ...
                          {{ $item->customer->fullName }}</span>
                                        ...
                                        <td class="h-px w-px whitespace-nowrap">
                    <span class="block px-6 py-2">
                        <span class="text-sm text-gray-600 dark:text-gray-400">{{ formatted_price($item->price, $item->currency) }}</span>
                    </span>
                                        </td>
                                        ...
                                        <td class="h-px w-px whitespace-nowrap px-6">
                                            <x-service-days-remaining expires_at="{{ $item->expires_at }}" state="{{ $item->status }}" date_at="{{ $item->status == 'expired' && $item->expire_at != null ? $item->expires_at->format('d-m-y') : ($item->suspended_at != null ? $item->suspended_at->format('d-m-y') : '') }}"></x-service-days-remaining>
                                        </td>
                      ...
                      <span class="text-sm text-gray-600 dark:text-gray-400">{{ $item->created_at->format('d/m/y') }}</span>
                                    ...
                                    <td class="h-px w-px whitespace-nowrap">
                                        ...
                                        <form method="POST" action="{{ route($routePath . '.show', ['service' => $item]) }}" class="inline" onsubmit="return confirmation()">
                                            @method('DELETE')
                                            @csrf
                                            <button>
