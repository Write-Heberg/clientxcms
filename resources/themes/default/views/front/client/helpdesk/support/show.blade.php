                    ...
                    <div class="card">
                        ...
                        <ul class="space-y-5">
                            @foreach ($ticket->messages as $i => $message)
                                ...
                                <!-- End Chat -->
                                <!-- Chat -->
                            <li class="{{ $message->containerClasses() }}">
                                <!-- Card -->
      ...
      <span class="me-3 flex-1 w-0 truncate">
         {{ Str::limit($attachment->filename, 30) }}
                            ...
                            <form method="POST" action="{{ route('front.support.close', ['ticket' => $ticket]) }}">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-secondary mt-2 w-full" type="submit">{{ __('client.support.show.close') }}</button>
                            </form>
                            @else
