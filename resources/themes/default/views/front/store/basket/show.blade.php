...
@section('content')
    ...
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
        ...
        <h1 class="text-2xl font-semibold mb-4 dark:text-dark">{{ __('store.basket.title') }}</h1>
                                ...
                                @foreach($basket->items()->get() as $row)
                                    ...
                                    <td class="py-4">
                                        <div class="flex items-center">
                                            ...
                                            <span class="font-semibold">{{ $row->product->name }}</span>
                                            @if ($row->primary() != null)
                                                <span class="text-primary ml-2">{{ $row->primary() }}</span>
                                                @endif
                                        </div>
                                    </td>
                                    <td class="py-4">{{ formatted_price($pricing->price,$pricing->currency)  }}</td>
                                    <td class="py-4">
                                        @if (!$row->product->getMetadata('disabled_many_services'))
                        ...
                        <div class="card mt-4 dark:text-gray-400">
                            ...
                            <form method="POST" action="{{ route('front.store.basket.coupon') }}">
                                ...
                                <div class="grid grid-cols-6 gap-2">
                                    ...
                                    <div>
                                        @if ($basket->coupon)
                                            <button type="submit" class="mt-2 btn-danger py-3 px-4 w-full"><i class="bi bi-x-circle mr-3"></i> {{ __('coupon.remove_coupon') }}</button>
                                        @else
                                            <button type="submit" class="mt-2 btn-primary py-3 px-4 w-full"><i class="bi bi-ticket-perforated mr-3"></i> {{ __('coupon.add_coupon') }}</button>
                                        @endif
                                    </div>
